...
class UsersController extends AppController
{
    ...
    public function add()
    {
        ...
        $subquery = $this->Users->find('list')->where([
            'Users.profesor_id = Profesores.id'
        ]);
        
        $profesores = $this->Users->Profesores->find('list')->where([
            'NOT EXISTS' => $subquery
        ]);
    ...
    public function login()
    {
        if ($this->request->is('post')) {
            ...
            if ($user) {
                ...
                $this->Flash->error('Datos invalidos, por favor intente nuevamente', [
                    'key' => 'auth'
                ]);
            }
        }
    ...
    public function perfil()
    {
        ...
        $pagos = TableRegistry::get('PagosAlumnos')->find()
            ...
            ->where([
            'DATE(PagosAlumnos.fecha_pagado)' => date('Y-m-d'),
            'PagosAlumnos.pagado' => true
        ])
        ...
        // return $this->redirect(['action' => 'graficoPdf','_ext' => 'pdf']);
    ...
    public function pPerfil()
    {
        ...
        $horarios = TableRegistry::get('Horarios')->find('all')
            ...
            return $q->where([
                'Clases.alumno_count >' => 0
            ]);
        })
            ->where([
            'nombre_dia' => date('l'),
            'Clases.profesor_id' => $id,
            'YEAR(ciclolectivo.fecha_inicio)' => date('Y')
        ])
            ->orderAsc("hora");
        $mensaje = '';
